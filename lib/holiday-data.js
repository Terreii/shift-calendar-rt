...
function getRamadan(year, month) {
  ...
  const start = new Date(year, startMonth, startDay, 12, 0, 0);
  const end = new Date(start.getTime() + ms.days(30));
  const idulFitr = new Date(end.getTime() + ms.days(1));
...
function getDaylightSavingDay(year, month) {
  ...
  const lastDay = new Date(year, month, 31).getDay();
